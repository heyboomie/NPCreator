...
def mainpart():
    ...
    Wstat = 0
    CHstat = 0
    races0 = ['Dwarf(Hill)', 0, 0, 2, 0, 1, 0, 'Dwarf(Mountain)', 2, 0, 2, 0, 0, 0, 'Elf(High)', 0, 2, 0, 1, 0, 0, 'Elf(Wood)', 0, 2, 0, 0, 1, 0, 'Elf(Drow)', 0, 2, 0, 0, 0, 1, 'Halfling(Lightfoot)', 0, 2, 0, 0, 0, 1, 'Halfling(Stout)', 0, 2, 1, 0, 0, 0, 'Human', 1, 1, 1, 1, 1, 1, 'Dragonborn', 2, 0, 0, 0, 0, 1, 'Gnome(Forest)', 0, 1, 0, 2, 0, 0, 'Gnome(Rock)', 0, 0, 1, 2, 0, 0, 'Half-Orc', 2, 0, 1, 0, 0, 0, 'Tiefling', 0, 0, 0, 1, 0, 2, 'Aasimar', 0, 0, 0, 0, 1, 2, 'Elf(Eladrin)', 0, 2, 0, 1, 0, 0, 'Aarakocra', 0, 2, 0, 0, 1, 0, 'Gnome(Deep)', 0, 1, 0, 2, 0, 0, 'Genasi(Air)', 0, 1, 2, 0, 0, 0, 'Genasi(Earth)', 1, 0, 2, 0, 0, 0, 'Genasi(Fire)', 0, 0, 2, 1, 0, 0, 'Genasi(Water)', 0, 0, 2, 0, 1, 0, 'Goliath', 2, 0, 1, 0, 0, 0, 'Dwarf(Gray/Duergar)', 1, 0, 2, 0, 0, 0, 'Halfling(Ghostwise)', 0, 2, 0, 0, 1, 0, 'Gnome(Deep,Svirfneblin)', 0, 1, 0, 2, 0, 0, 'Tiefling(Feral)', 0, 2, 0, 1, 0, 0, 'Aasimar(Protector)', 0, 0, 0, 0, 1, 2, 'Aasimar(Scourge)', 0, 0, 1, 0, 0, 2, 'Aasimar(Fallen)', 1, 0, 0, 0, 0, 2, 'Firbolg', 1, 0, 0, 0, 2, 0] 
    #print(races0)
    racespoint5 = ['Goliath', 2, 0, 1, 0, 0, 0, 'Kenku', 0, 2, 0, 0, 1, 0, 'Lizard folk', 0, 0, 2, 0, 1, 0, 'Tabaxi', 0, 2, 0, 0, 0, 1, 'Triton', 1, 0, 1, 0, 0, 1, 'Bugbear', 2, 1, 0, 0, 0, 0, 'Goblin', 0, 2, 1, 0, 0, 0, 'Hobgoblin', 0, 0, 2, 1, 0, 0, 'Kobold', 0, 2, 0, 0, 0, 0, 'Orc', 2, 0, 1, 0, 0, 0, 'Yuan-ti Pureblood', 0, 0, 0, 1, 0, 2, 'Elf(Eladrin)', 0, 2, 0, 0, 0, 1, 'Elf(Sea)', 0, 2, 1, 0, 0, 0, 'Elf(Shadar-kai)', 0, 2, 1, 0, 0, 0, 'Dwarf(Duergar)', 1, 0, 2, 0, 0, 0, 'Githyanki', 2, 0, 0, 1, 0, 0, 'Githzerai', 0, 0, 0, 1, 2, 0, 'Tortle', 2, 0, 0, 0, 1, 0, 'Verdan', 0, 0, 1, 0, 0, 2, 'Kalashtar', 0, 0, 0, 0, 2, 1, 'Shifter(Beasthide)', 1, 0, 2, 0, 0, 0]
    races1 = ['Shifter(Longtooth)', 2, 1, 0, 0, 0, 0, 'Shifter(Swiftstride)', 0, 2, 0, 0, 0, 1, 'Swiftstride(Wildhunt)', 0, 1, 0, 0, 2, 0, 'Centaur', 2, 0, 0, 0, 1, 0, 'Loxodon', 0, 0, 2, 0, 1, 0, 'Minotaur', 2, 0, 1, 0, 0, 0, 'Vedalken', 0, 0, 0, 2, 1, 0, 'Leonin', 1, 0, 2, 0, 0, 0, 'Satyr', 0, 1, 0, 0, 0, 2]
    races = races0 + racespoint5 + races1
    #print(races)
    npcname = "placeholder"
...
#--------------------------------------------------------------
    ...
    #time.sleep(1)
    #cwd = os.getcwd()
    #cwdirt1 = cwd + "\\RacialBonuses1.txt"
    #cwdirt2 = cwd + "\\RacialBonuses2.txt"
    #cwdirt3 = cwd + "\\RacialBonuses3.txt"
    #cwdirt4 = cwd + "\\RacialBonuses4.txt"
    #with open(cwdirt1) as f:
    #    rases = f.readlines()
    #print("loading races")
    #time.sleep(.5)
    #with open(cwdirt2) as f:
    #    rases2 = rases + f.readlines()
    #print("loading races.")
    #time.sleep(.5)
    #with open(cwdirt3) as f:
    #    rases3 = rases2 + f.readlines()
    #print("loading races..")
    #time.sleep(.5)
    #with open(cwdirt4) as f:
    #    rases4 = rases3 + f.readlines()
    #print("loading races...")
    #time.sleep(.5)
    #races = [h[:-1] for h in rases4]
    time.sleep(.5)
    print("loading races")
    time.sleep(.3)
    print("loading races.")
    ...
    time.sleep(.3)
    #print("test done"
    print("(note: there are 67 races you can chose from as found on https://summoninggrounds.com/5e-racial-stat-bonus-chart/)")
    ...
    while(looppusher == 1):
        if(npcrace == races[v] and v < 442):
            ...
            Sbonus2 = int(Sbonus3)
            Dbonus2 = int(Dbonus3)
            CObonus2 = int(CObonus3)
            Ibonus2 = int(Ibonus3)
            Wbonus2 = int(Wbonus3)
            CHbonus2 = int(CHbonus3)
            break
        elif(npcrace != races[v]):
            v=v+7
        elif(npcrace == "Changeling"):
            CHbonus2 = 2
            ...
            if(CustomBonus == 'S'):
                ...
                print(howmanyarepicked, " has been picked")
                if(howmanyarepicked == 2):
                    break
            elif(CustomBonus == 'D'):
                Dbonus2 = 1
                howmanyarepicked = howmanyarepicked + 1
...
#------------------------------------------------------------
    ...
    Connpcrace = str(npcrace)
    line0 = npcname + " is a ", Connpcrace
...
restart = ""
while(looppusher2 == 1):
    print ("Welcome To DnD NPC stat generator v1.8")
    ...
    while(looppusher2 == 1):
        ...
        if start_key == ("s"):
            ...
            #loadingtext()
            mainpart()
            print("Thanks for using DnD NPC stat generator v1.8")                
    #thanks_for_using
    #-----------------------------------------------------------------